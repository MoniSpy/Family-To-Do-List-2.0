...
async function getUserItems(userId) {
  const db = await getDb();
  const result = await db.query(
    'SELECT items.id , text, creation_date, lists_id, users_id FROM items JOIN users ON users.id = users_id WHERE users_id =($1) ORDER BY items.id ASC',
    [userId]
  );
...
async function getUserLists(userId) {
  const db = await getDb();
  const result = await db.query(
    'SELECT lists.id ,lists_name, user_id FROM lists JOIN users ON users.id=lists.user_id WHERE users.id=$1;',
    [userId]
  );
